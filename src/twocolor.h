...
public:
  ...
  // Управление двухцветным светодиодом. Использует константы вида: Off, Green, Red
  static void control(TwoColorLedColor color);

  // Задать двухцветному светодиоду красный цвет. Инвертируется
  static void setRed();

  // Задать двухцветному светодиоду зелёный цвет. Инвертируется
  ...
  // Мигать двухцветным светодиодом с указанными цветом, временем
  static void blink(TwoColorLedColor color, int time);

  //! @param p1 - Первый пин двухцветного светодиода
  //! @param p2 -
  // Задать пины для двухцветного светодиода. По умолчанию 3 и 6
